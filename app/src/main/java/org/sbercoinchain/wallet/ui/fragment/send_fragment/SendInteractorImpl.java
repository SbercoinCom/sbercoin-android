...
public class SendInteractorImpl implements SendInteractor {
    ...
    public BigDecimal getFeePerKb() {
        ...
        return new BigDecimal(sbercoinSettingSharedPreference.getFeePerKb(mContext));
    }
    ...
    public void getUnspentOutputs(String address, final GetUnspentListCallBack callBack) {
        ...
        SBERService.newInstance().getUnspentOutputs(address)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<List<UnspentOutput>>() {
                    @Override
        ...
        getUnspentOutputs(from, new GetUnspentListCallBack() {
            ...
            public void onSuccess(List<UnspentOutput> unspentOutputs) {
                ...
                try {
                    ...
                    callBack.onError(mContext.getString(org.sbercoin.wallet.R.string.invalid_sbercoin_address));
                    return;
                }
    ...
    public Observable<CallSmartContractResponse> callSmartContractObservable(Token token, String hash, String fromAddress) {
        return SBERService.newInstance().callSmartContract(token.getContractAddress(), new CallSmartContractRequest(new String[]{hash}, fromAddress));
    }

    @Override
    public int getMinGasPrice() {
        return SBERSharedPreference.getInstance().getMinGasPrice(mContext);
    }
}