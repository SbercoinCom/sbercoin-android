...
public class MainActivity extends BaseActivity implements MainActivityView {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent != null && intent.getAction() != null) {
            switch (intent.getAction()) {
                case BCSIntent.OPEN_FROM_NOTIFICATION:
                    openRootWallet();
                    break;
                default:
                    break;
    ...
    public boolean checkTouchIdEnable() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (BCSSharedPreference.getInstance().isTouchIdEnable(getContext())) {
    ...
    public void initializeViews() {
        ...
        switch (intent.getAction()) {
            case BCSIntent.SEND_FROM_SDK:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = intent.getStringExtra(BCSIntent.SEND_ADDRESS);
                mAmountForSendAction = intent.getStringExtra(BCSIntent.SEND_AMOUNT);
                mTokenAddressForSendAction = intent.getStringExtra(BCSIntent.SEND_TOKEN);
                break;
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(CustomContextWrapper.wrap(newBase, BCSSettingSharedPreference.getInstance().getLanguage(newBase)));
    }

    public BCSApplication getBCSApplication() {
        return (BCSApplication) getApplication();
    }
    ...
    public Realm getRealm(){
        return ((BCSApplication)getApplication()).getRealm();
    }
}