...
public class SendInteractorImpl implements SendInteractor {
    ...
    public void getUnspentOutputs(final GetUnspentListCallBack callBack) {
        BCSService.newInstance().getUnspentOutputsForSeveralAddresses(KeyStorage.getInstance().getAddresses())
    ...
    public void getUnspentOutputs(String address, final GetUnspentListCallBack callBack) {
        ...
        BCSService.newInstance().getUnspentOutputs(address)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<List<UnspentOutput>>() {
                    @Override
        ...
        getUnspentOutputs(from, new GetUnspentListCallBack() {
            ...
            public void onSuccess(List<UnspentOutput> unspentOutputs) {
                ...
                try {
                    ...
                    callBack.onError(mContext.getString(org.sbercoin.wallet.R.string.invalid_bcs_address));
                    return;
                }
                ...
                if (overFlow.doubleValue() < amount.doubleValue()) {
                    callBack.onError(mContext.getString(org.sbercoin.wallet.R.string.you_have_insufficient_funds_for_this_transaction));
                    return;
                }
                ...
                if (minimumFee.doubleValue() > fee.doubleValue()) {
                    callBack.onError(mContext.getString(R.string.insufficient_fee_lease_use_minimum_of) + " " + minimumFee.toString() + " SBER");
                    return;
                }
    ...
    public void sendTx(String txHex, final SendTxCallBack callBack) {
        BCSService.newInstance().sendRawTransaction(new SendRawTransactionRequest(txHex, 1))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    public String getValidatedFee(Double fee) {
        ...
        return decimalFormat.format(fee).replace(",",".");
    }
    ...
    public Observable<CallSmartContractResponse> callSmartContractObservable(Token token, String hash, String fromAddress) {
        return BCSService.newInstance().callSmartContract(token.getContractAddress(), new CallSmartContractRequest(new String[]{hash}, fromAddress));
    }
