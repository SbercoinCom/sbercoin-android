...
import static org.sbercoin.wallet.utils.crypto.KeyStoreHelper.BCS_PIN_ALIAS;
...
public class MigrationManager {
    ...
    private static KeystoreMigrationResult migratePassphrase(Context context) {
        if (TextUtils.isEmpty(BCSSharedPreference.getInstance().getKeystoreMigrationState(context))) {
            String encryptedSaltPassphrase = BCSSharedPreference.getInstance().getSeed(context);
            if (!TextUtils.isEmpty(encryptedSaltPassphrase)) {
                try {
                    KeyStoreHelper.createKeys(context, BCS_PIN_ALIAS);
                    byte[] bytes = KeyStoreHelper.decryptToBytes(BCS_PIN_ALIAS, encryptedSaltPassphrase);
                    savePassphraseSalt(context, bytes);
                    BCSSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.SUCCESS.name());
                    return KeystoreMigrationResult.SUCCESS;
                } catch (NoSuchProviderException | NoSuchAlgorithmException | InvalidAlgorithmParameterException | RuntimeException e) {
                    return KeystoreMigrationResult.ERROR;
                }
            } else {
                BCSSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.NOT_NEED.name());
                return KeystoreMigrationResult.NOT_NEED;
            }
        } else {
            return KeystoreMigrationResult.NOT_NEED;
    ...
    private static void migratePinCode(Context context) {
        boolean isSixDigit;
        String encryptedPinHash = BCSSharedPreference.getInstance().getSixDigitPassword(context);
        isSixDigit = !TextUtils.isEmpty(encryptedPinHash);
        if (!isSixDigit) {
            encryptedPinHash = BCSSharedPreference.getInstance().getPassword(context);
            if (TextUtils.isEmpty(encryptedPinHash)) {
                return;
            }
        }
        String decrypt = KeyStoreHelper.decrypt(BCS_PIN_ALIAS, encryptedPinHash);
        if (isSixDigit) {
            BCSSharedPreference.getInstance().saveSixDigitPassword(context, decrypt);
        } else {
            BCSSharedPreference.getInstance().savePassword(context, decrypt);
        }
    }
    ...
    private static void savePassphraseSalt(Context context, byte[] saltPassphrase) {
        ...
        BCSSharedPreference.getInstance().saveSeed(context, base64);
    }

    private void clearWallet(Context context, Realm realm) {
        BCSSharedPreference.getInstance().forceClear(context);
