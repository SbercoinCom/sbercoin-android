...
import static org.sbercoin.wallet.BCSApplication.REALM_NAME;
...
public class WearListCallListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, MessageApi.MessageListener {
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if(messageEvent.getPath().contains("/get_history")) {
            ...
            currentAddress = BCSSharedPreference.getInstance().getCurrentAddress(getApplicationContext());
            sendData();
        } else {
            stopSelf();
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    final String balance = BCSSharedPreference.getInstance().getBalanceString(WearListCallListenerService.this);
                    final String uncBalance = BCSSharedPreference.getInstance().getUnconfirmedBalanceString(WearListCallListenerService.this);
                    final String address = BCSSharedPreference.getInstance().getCurrentAddress(WearListCallListenerService.this);
                    Gson gson = new Gson();
